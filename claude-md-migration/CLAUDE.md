...
# or: chainlit run claudia/app.py
...
- **API Docs First**: never assume endpoint behavior, error codes, or field names from
  ...
  docs were checked. Full source table: @docs/api-references.md
...
- Prompt caching uses 3 breakpoints (tools → system → messages). Design rationale and
  live-verified numbers: @docs/superpowers/plans/2026-07-03-prompt-caching-upgrade.md
...
- **Order parameters are immutable**: ClaudIA must use the user's exact values (symbol,
  ...
  parameter gets a text warning, never a silent change. Enforced in `claudia/agent.py`
  system prompt and in memory (`feedback-order-parameter-immutability.md`).
...
- API source-of-truth URLs (IBKR, Anthropic, Drive, Chainlit, libraries): @docs/api-references.md
