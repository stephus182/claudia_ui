...
| `CRAWL4AI_API_KEY` | optional | Crawl4AI **Cloud** API key (`sk_live_…`) — enables the web-scraper ladder's third rung (`ibkr_core_mcp/crawl4ai_cloud.py`), reached only after Firecrawl *and* the free local scraper both come up short. Unset ⇒ the rung is skipped silently and the ladder behaves as it did with two rungs. Distinct from `CRAWL4AI_PROFILES_DIR`, which configures the *local* scraper. ClaudIA keeps its own key, separate from ibkr_core_mcp's — deliberate, not duplication. Was named `SCRAWL4AI_API_KEY` until 2026-07-28; under that name nothing read it, so the rung sat dark while appearing configured. |
