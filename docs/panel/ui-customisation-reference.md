...
| TradingView | launch Desktop with the debug port if it is not up, rebuild the sidecar bridge, hand the tools to the agent, then re-poll the checker. **2026-09-04:** a launch that never produces a TradingView process is reported as "never started" with the Terminal route — a ClaudIA server started from a Claude Code tool shell cannot launch desktop apps at all (`docs/tradingview-reference.md` § Normal startup, step 4); a running app without the port keeps the quit+relaunch instructions |
