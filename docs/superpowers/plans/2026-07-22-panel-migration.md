...
local_path, filename)` (`cache.py:373`); `toolkit._cache` exists (constructor-set,
`claude_tools.py:1029`). Parity source: `app.py:550-617`. `ibkr_offline` comes from
Task 5.6b's `_send_opening_status` return.

**Design (locked at detailing):**

- One module-level async `_maybe_background_flex_sync(chat, toolkit, ibkr_offline)
  -> None`: runs the three-step DECISION (fast sqlite via to_thread) inline, and only
  spawns the long-running sync as a fire-and-forget task (strong ref in a module-level
  `_background_tasks: set[asyncio.Task[None]]`, done-callback discard + failure
  logging — same shape as `_cleanup_tasks`). `_init_session` awaits the decision
  function as its last act (after the buttons message), so init latency gains only the
  two sqlite reads.
- Decision port is exact app.py:554-581 logic: skip if `not flex_configured or
  ibkr_offline`; skip with reason if coverage not stale; if stale, check
  `get_log(n=1, event="flex_sync")` — skip if last attempt < 4h; any check exception →
  attempt sync.
- Sync body port (app.py:584-617): `toolkit.execute("sync_flex_trades", {})` via
  to_thread → `chat.send(f"✅ {result}", ...)`; then store.db backup via
  `toolkit._cache.upload_account_file(cfg.sqlite_path, "store.db")` (to_thread,
  non-fatal warn). On sync failure: `check_flex_coverage` fallback message, else the
  plain ⚠ message. `chat.send` on a since-closed session is a harmless no-op (V4
  probe knowledge) — no extra guarding.
- app.py's `assert _config is not None` dance disappears: panel uses
  `toolkit._config` directly (same sanctioned reach-in as 5.3/5.5).
- Test seam: the autouse `backend_singletons` fixture ALSO patches
  `_maybe_background_flex_sync` (AsyncMock) and yields it as `.flex_sync` — the 27
  existing tests stay untouched; the real function gets direct unit tests; one wiring
  test asserts `_init_session` awaited the patched seam with the gather-derived
  `ibkr_offline`.

**Files:**

- Modify: `claudia/panel_app.py` (imports `datetime`/`UTC`; `_background_tasks`;
...
stay untouched. The clean shape is a marker-conditional autouse fixture (NOT the
`backend_singletons` extension sketched above — replace that sketch with this):
...
(register the marker in pyproject `[tool.pytest.ini_options]` `markers`). Unit tests
then carry `@pytest.mark.real_flex_sync` and call the real function directly.
...
`_init_session`, after `_send_action_buttons(...)`:

```python
            await _maybe_background_flex_sync(chat, toolkit, ibkr_offline)
```

pyproject `[tool.pytest.ini_options]` gains
...
reads — verify no flex log lines). This smoke can't reach the sync path (needs live
...
git commit -m "feat: Panel background Flex sync + store.db Drive backup (app.py parity, decision seam)"
```
