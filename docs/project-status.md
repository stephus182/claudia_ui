...
| 2026-07-17 | `871e3c8`…`78b7b08` (10 commits, merged) | **IBKR soft-timeout silent recovery** — when the gateway's brokerage session soft-times-out (`connected:true, authenticated:false`), `ConnectivityChecker` now silently re-establishes it via IBKR's documented `POST /iserver/auth/ssodh/init` instead of always forcing a manual browser+2FA re-login, gated behind a narrow safety condition (only fires on a transition from a previously-confirmed `OK` state — never from `UNKNOWN`, which covers the fragile fresh-login window, and never on a hard disconnect). Built via subagent-driven TDD, full plan at `docs/plans/2026-07-17-ibkr-soft-timeout-recovery.md`: Task 1 (`871e3c8`/`ff5e291`) stashes the full IBKR auth-status dict during `check_ibkr()`; Task 2 (`2dbfe99`) adds `_attempt_soft_recovery()` standalone; Task 3 (`8a0795d`/`d50c530`/`e464eae`) wires it into `_run_checks()`; Task 4 (`2e4a2de`) updates `docs/connectivity.md`. Two safety-relevant gaps caught by review and fixed before merge: `_attempt_soft_recovery()` originally trusted HTTP 200 alone as success — fixed (`fb560b8`) to also check the response body's `authenticated` field, since IBKR returns 200 regardless of outcome on related endpoints (the same lesson `check_ibkr()` already encoded); and a doc sweep (`7868752`/`216814d`/`cd8dd76`) caught two other docs still saying "not yet implemented" after Task 4 only updated one, plus a dangling reference to the plan doc (fixed by committing it to the branch — it had only ever existed as an untracked file). 15 dedicated unit tests covering every safety-relevant branch (never fires from `UNKNOWN`/`ERROR`/hard-disconnect; successful recovery suppresses the disconnect alert entirely; failed recovery — including a "successful" POST whose immediate re-check still fails differently — produces exactly one normal alert, same as pre-existing behavior). Full suite: 298→313. **Not yet live-verified** — needs a human physically present with a live gateway session left deliberately idle past ~6 minutes; see the plan's Task 5 for the safety-scoped protocol (framed so the worst case is identical to today's status quo, since the soft-timeout has already occurred naturally by the time recovery is tested). |
| 2026-07-22 | `2e1d3f0`…`de1ccf5` (5 commits) | **Pre-migration code-quality audit** — first `mypy`/expanded-`ruff` pass this project has ever run: 91 fresh mypy errors (not the stale, never-itemized "~102" figure quoted in the 2026-07-08/07-10 rows above) and 106 ruff findings under a new, justified rule set, both resolved to 0 — mostly via genuine root-cause fixes (one narrowing fix in `agent.py`'s streaming loop alone resolved 74 of the 91), the rest via narrow, individually-reasoned `type: ignore`/`noqa` exceptions where a fix would have been disproportionate. Also fixed: `ibkr_core_mcp`'s editable install was entirely invisible to mypy (needed `--config-settings editable_mode=strict`, now documented in `CLAUDE.md`); `mcp` was pinned unbounded despite its own README naming v2 "pre-release... do not use in production" (now `>=1.27,<2`). All 10 unsourced `api-reference.md` URLs and all 10 open Pending Doc Verification items resolved against real scraped IBKR docs (8 confirmed, 5 corrected — including this doc's own stale "21-code" Flex error-table claim, corrected to 20 — 2 confirmed genuinely undocumented by IBKR). 313 tests green throughout, zero behavior changes to the order-staging/proposal-parsing safety-critical surface (re-checked against all 5 CLAUDE.md hard rules on every touch). Full report: [`docs/audits/2026-07-22-code-quality-pre-migration-audit.md`](audits/2026-07-22-code-quality-pre-migration-audit.md). |

---

## Test Coverage

**Suite:** 313 tests, 0 failures (non-integration). Run: `pytest -m "not integration" -q`
(this header was stale at 295 — the 2026-07-17 soft-timeout-recovery Feature Timeline
entry already logged 298→313 five days before this correction; reproduced fresh
2026-07-22 in an isolated worktree venv, unchanged throughout the code-quality audit)
...
1. ~~Account P&L / Account Summary display bugs~~ — **✅ DONE AND LIVE-VERIFIED 2026-07-17**, all
   3 tasks of `docs/plans/2026-07-17-account-pnl-display-fixes.md`
...
**2026-07-18 live test session plan: `docs/plans/2026-07-18-live-test-session.md`.**
...
**Goal:** Batch 1 Order Operations (send/modify/cancel/reply) first, then TradingView, price alerts, security. Full plan: `docs/plans/2026-07-08-order-cancel-modify.md` — Part B is the order cancel/modify UI wiring build, Part C is this batch plan.
...
`docs/plans/2026-07-10-live-test-bugfixes-handoff.md`.
...
| `GDriveSync.upload_db` swallows its own errors — cleanup paths can report Drive success on a real failure | `claudia/gdrive_sync.py` (`upload_db`, lines 249-306), `claudia/panel_app.py` (`_run_session_cleanup`, `main()` finally), `claudia/app.py` (`_run_session_cleanup`) | Documented during the Task 5.6b quality review (2026-07-24, Panel migration). `upload_db()` returns `None` and catches every exception internally (logs "upload_db failed … local copy preserved" and returns normally), so the callers' own try/except never fires on a real Drive failure — both session-end cleanup paths can render "claudia.db → Drive ✅" and shutdown can log "Final claudia.db upload complete" when nothing was actually uploaded. Byte-parity with the Chainlit `app.py` cleanup — pre-existing behavior, not a Panel-migration regression. Upstream fix when scheduled: make `upload_db` return `bool` (symmetric with `download_db`) and thread the result into both entry points' status strings/logs. |
...
| Empty trades table skips the first-ever startup Flex sync with a misleading "data current" log | `ibkr_core_mcp/store.py` (`get_trade_date_coverage`, line 355), `claudia/panel_app.py` (`_maybe_background_flex_sync`), `claudia/app.py` (startup Flex-sync decision) | Documented during the Task 5.7 quality review (2026-07-24, Panel migration). With zero rows in `trades`, `get_trade_date_coverage` returns `{"oldest": None, "newest": None, "total_trades": 0, "gaps": []}` — no `stale` key at all (store.py:355) — so the sync decision's `cov.get("stale")` is falsy and a NEVER-populated store is treated as current: the first-ever sync is skipped and the log misleadingly reads "data current (newest: None, last trading day: None)". Exact app.py parity — pre-existing behavior, not a Panel-migration regression. Upstream `ibkr_core_mcp` fix candidate: treat a missing/None `newest` as stale in `get_trade_date_coverage`. |

### Unfixed issues carried out of the Chainlit→Panel migration (2026-07-24) — future work

Consolidated register of everything found but deliberately NOT fixed during the migration
(each was recorded at find time; gathered here so nothing relies on the local plan archive).
Already tracked as their own rows above and not repeated: `upload_db` swallowed errors,
empty-trades Flex-sync skip, and the proposal-button omission (see its **2026-07-24 update**
— now reproduced in a fresh session; the anti-fabrication guardrail spec is the top item).

| Item | File | Status |
|---|---|---|
| **Anti-fabrication guardrail spec — not yet written** | `claudia/agent.py` (agent loop, post-response checks) | The primary fix for the proposal-button-omission row above (and the same class as the 2026-07-10 retry-fabrication findings). Two detections: (a) proposal-intent-without-block → honest retry/re-ask; (b) data-question-without-tool-call → forced tool use. Deliberately deferred out of the migration (agent.py was frozen during Phases 8-10). Write the spec as a dated design doc, then subagent-driven TDD. |
| Live chart fetch→render smoke not yet run | `claudia/panel_chart.py` | Task 10.1's Step-4 manual smoke: the candlestick *render* is proven on real cached OHLCV (60-bar QQQ screenshot) and the fetch wiring is unit-tested, but the end-to-end live fetch→render (plus bogus-symbol and IBKR-offline honest-error eyeball) needs an **authenticated gateway** — the session had dropped at smoke time and was not re-authed per `feedback-ibkr-session-safety`. Run next authenticated session. |
| Chart "No data" message conflates IBKR-offline vs unknown-symbol | `claudia/panel_chart.py` (`_on_load`) | Code-quality review minor (Task 10.1): `toolkit.execute("fetch_market_data", …)`'s returned SUMMARY string is discarded, so every miss shows the same `"No data for {sym}."`. Capturing the summary into the status line would be strictly more honest — but first verify `execute`'s real return type per API-Docs-First (the mock in tests returns a tuple; the real shape must be confirmed before use). |
| `TradingViewBridge.start()` handshake is unbounded | `claudia/tradingview.py` (`start`) | The MCP stdio handshake has no timeout — a wedged sidecar process could stall session init indefinitely. Deferred fix: wrap the handshake in `asyncio.wait_for` with a bounded timeout + honest degradation to screenshot mode. Never observed hanging live (sidecar either starts or exits fast), hence low priority. |
| Chart first-cut scope limits (by design, not defects) | `claudia/panel_chart.py` | Deferred to the restyle/future bucket, recorded in the Phase-10 research: volume subplot, MA/indicator overlays, crosshair/hover, zoom-sync, multi-symbol, non-STK (FUT/OPT) charts, theme-matched colors, freeform period input. |
| Cross-repo `ibkr_core_mcp` follow-ups from migration testing | `ibkr_core_mcp` (other repo) | (1) `get_watchlists` nested-dict handling bug observed during live testing; (2) `place_order` docstring still implies `extOperator` should be sent for CME 536-B — reversed by the live-proven field-8089 finding (IBKR rejects it; `manualIndicator`-only). Both belong upstream, out of this repo's scope (same treatment as the FUT/FOP cancel-params row above). |
| Post-migration deep restyle — dedicated plan pending | Panel UI (all `panel_*` modules) | Phase 7 was deliberately reduced to minimal styling (function-first ruling). The deep "touch"/feel restyle is a separate future project with its own specs, drawing on `docs/panel/` research. Includes revisiting the chat/chart split ratio vs tabs, theme, and the deferred chart features above. |
