...
| Terminal-style log: `pn.Column(scroll=True, view_latest=True, height=…)` of one `pn.pane.Str` per line | Shipped 2026-09-04 (`panel_system_log.py`). `Str` is Panel's literal-text pane (monospace `<pre>`, everything escaped) — the natural fit for log lines, and constructed only via `panel_markdown.safe_text` so the H-1 structural test keeps one sanctioned site. The lines carry `stylesheets=["pre { white-space: pre-wrap; margin: 0; }"]` so a long gateway detail wraps instead of clipping — `styles={"white-space": …}` was tried first and does nothing here: it lands on the pane's wrapper and the browser's own `pre { white-space: pre }` wins (measured 2026-09-04); the rule has to reach the `<pre>`, which only the component's shadow-scoped stylesheet does (§2). **First component stylesheet in the codebase.** Replaced a `ChatFeed` of bubbles after one day: avatar + name + padding per entry was chat, not log |
