...
- [ ] **B6 — Read-back timing** (L2, new): after staging, check whether `_READBACK_DELAY_S
      = 2.0` is long enough for a fresh order to report a working status rather than
      `PendingSubmit`. If it is too short, users get honest-but-noisy "not confirmed" on
      good orders — the fix is tuning the constant, not the logic. Also eyeball the modify
      read-back's rendered output: the price field cannot be verified discretely (IBKR
      exposes no discrete limit-price field on order status), so a ✅ line and a ⚠️ price
      caveat appear together
