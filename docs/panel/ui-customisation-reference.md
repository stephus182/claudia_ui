...
| System log (2026-09-03; terminal-style since 2026-09-04) | a collapsed `pn.Card` titled "System log (n)" under the chat input. Body: a scrolling `pn.Column` (`scroll=True`, `view_latest=True`, 240 px) of one monospace line per event — `HH:MM:SS  [WARN  \|ERR   ]text`, Markdown marks dropped, newlines collapsed (`panel_system_log.format_line`), each line a `panel_markdown.safe_text` (`Str` pane: raw string, escaped by construction). Every **session-level** event lands there (§2.6 has the routing rule) and `warning`/`error` lines also raise a toast. The chat keeps only the conversation and the per-turn feedback that answers something you just did. It was a `ChatFeed` of bubbles for one day; the user asked for "terminal style lines vs chat style" and the bubbles' avatar/name/padding went | `panel_system_log.SystemLog`; every call site is `syslog.say(...)` in `panel_app` (`_build_action_bar`, `_maybe_background_flex_sync`, `_init_session`, `_make_alert_subscriber`) | — | `_FEED_HEIGHT` for the pane height; `_LEVEL_TAG` for the level column; route a call back to `chat.send(..., user="System")` and the source-scan test `test_no_session_level_system_send_reaches_the_chat_feed` will name it |
| Action bar (2026-09-03) | `[IBKR] [TradingView] [Drive] [End Session]` at the bottom of the chat column, replacing the three status dots **and** the "System" message that used to carry the buttons. **Colour is the light**: `success` green = up, `danger` red = down, `default` (white, real border) = not configured / not checked yet — **not `light`**: Bokeh's `.bk-btn-light` sets `border-color: transparent`, so End Session and the neutral state melted into the page (user 2026-09-04, fixed the same morning; `2026-09-04-action-bar-default-style-states.png`). **A click reconnects; it never merely checks** (user rule). Repainted every 5 s from the checker's cache, and again right after a reconnect | `panel_action_bar.ActionBar` (`SERVICE_LABELS` for order and labels, `_COLOR` for the mapping); the three reconnect coroutines in `panel_app._build_action_bar` | — | there is no "revert" short of restoring the dots from git — the bar *is* the status display now |
| Order-card buttons (2026-09-24, gap #67) | place `STAGE ORDER` (blue) / `DISCARD` (red); cancel `CANCEL ORDER` (red) / `KEEP ORDER` (neutral); modify `MODIFY ORDER` (blue) / `DISCARD` (red). UPPERCASE, no articles, matching Gate 2's labels. **Blue validates, red throws away or removes, neutral leaves in place; one label, one colour; "CANCEL" only on the button that cancels a live order.** Agreed with the operator one button at a time. Deliberate departures from Apple's HIG: no "Cancel" dismiss button (an order verb here), and uppercase rather than title case | labels: `order_flow.*_LABEL` (also used by each card's instruction line); colours: `panel_order_flow._ORDER_BUTTON_COLORS`, built only through `_order_button` | — | never edit one card's colour inline: change the table, and `tests/test_panel_order_flow.py` (which clicks each button by its label) says whether the rules still hold |
...
| Lights | at first paint IBKR **red**, TradingView neutral, Drive green; IBKR **green by 20:17:37** after the checker's second poll. The red start is the checker's first poll racing the session owner's first read — the same latency the dots had; a candidate in §4 |
...
`ERR` + sticky toast (`panel_system_log._TOAST_DURATION_MS`, `_LEVEL_TAG`). The count in the
card title is the whole "unread" mechanism — deliberately no badge, no auto-expand. The toast
carries the original Markdown text; the line carries the same words without the marks.
...
| TradingView | the same action as `scripts/launch-tradingview-debug.sh`: quit a TradingView running without the debug port, launch it with the port, wait for the process then the port, rebuild the sidecar bridge, hand the tools to the agent, then re-poll the checker; every progress line lands in the System log. **2026-09-04:** a launch that never produces a TradingView process is reported as "never started" with the Terminal route — a ClaudIA server started from a Claude Code tool shell cannot launch desktop apps at all (`docs/tradingview-reference.md` § Normal startup, step 4); a running app without the port keeps the quit+relaunch instructions |
